...
function SecondYear() {
    const [data, setData] = useState([]); // Stores all 2nd Year modules
    ...
    // Fetch 2nd Year data when the component mounts
    useEffect(() => {
        fetchData();
    }, []);

    // Fetch data for 2nd Year
    const fetchData = async () => {
        setLoading(true);
        setError("");

        try {
            const response = await axios.get(`http://localhost:8081/api/docs/year/2nd Year`);
            setData(response.data);
            setFilteredData(response.data); // Initialize filtered data with all 2nd Year modules
        } catch (error) {
